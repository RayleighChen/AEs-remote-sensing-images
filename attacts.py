...
def record(model, class_indices):
	...
	preprocessing = dict(mean=model.mean, std=model.std, axis=-3)
	...
	for batch_files in yield_mb(files, args.batch_size, shuffle=False):
		...
		for i in range(args.batch_size):
			...
			ads_record.write("{_class}+{file_name}+{model_cfs:.2f}+{ads_cla}+{ads_cfs:.2f}.jpg,{_class},{model_cla},{model_idx},{model_cfs:.2f},{model_flg},{ads_cla},{ads_idx},{ads_cfs:.2f},{ads_flg}\n".format(_class=_class, file_name=file_name, model_idx=model_idx, model_cla=model_cla, model_cfs=model_cfs, model_flg=model_flg,					ads_idx=ads_idx, ads_cla=ads_cla, ads_cfs=ads_cfs, ads_flg=ads_flg))
			
			if model_flg and ads_flg:
				image_path = os.path.join(ads_path, _class)
				imageio.imwrite("{image_path}/{_class}+{file_name}+{model_cfs:.2f}+{ads_cla}+{ads_cfs:.2f}.jpg".format(image_path=image_path,_class=_class, file_name=file_name,model_cfs=model_cfs,ads_cla=ads_cla,ads_cfs=ads_cfs), img_as_ubyte(np.transpose(adversarials[i], (1, 2, 0))))
